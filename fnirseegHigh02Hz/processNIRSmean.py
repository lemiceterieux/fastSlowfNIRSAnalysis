...
def doMBL(c, eps=[[.974, .693],[.35,2.1]], DPF=[5.98, 7.15], sd=2.5, filt=False, ica=False, embed=None, tau=None, mrk=None, oxy="", subj="", trial=""):
    ...
    if True:#not os.path.isfile("beerlamb_{0}_{1}.npy".format(subj,trial)):
        ...
        if filt:
            ...
            b2,a2 = signal.butter(3,[.2/nyq, .8/nyq], "band")
...
#        temp = torch.from_numpy(beerlamb.reshape(-1,beerlamb.shape[-1]).T)
#        lag = 40
#        print(temp.shape,"Making lag matrix")
#        tlag = torch.stack([temp[i:-lag+i] for i in range(lag)])
#        tlag = tlag.reshape(lag,len(temp)-lag,-1).permute(2,1,0)
#        tnews = []
#        for t in tlag:
#            tcov = t.T.matmul(t)
#            U,S,V = torch.linalg.svd(tcov)
#            signs = abs(torch.sign(V).sum(-1)) == lag
#            for j in range(lag):
#                if signs[j]:
#                    V[j] = 0
#            tt = t.matmul(V.T).matmul(V)
#            trec = torch.cat([tt[:,0]] + [tt[[-1],i] for i in range(1,lag)],-1)
#            tnews += [trec.numpy()]
#        beerlamb = np.array(tnews).reshape(36,2,temp.shape[0]-1)
        temp = beerlamb.reshape(-1,beerlamb.shape[-1]).T
        import torch
        W = torch.rand(72,72).float().requires_grad_()
